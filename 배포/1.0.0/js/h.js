...
/*/////////////////////////////////////////////////////////////////
	...
	* 		- JavaScript-XPath 0.1.12 (javascript-xpath-latest.js)
	* 
...
(function($){
	...
	H.prototype = {
			...
			_getSelectRange: function(idx){
				idx = idx || 0;
				// all browsers, except IE before version 9
				var selectRange = this._selection.getRangeAt(idx);
				return selectRange;
			},
			...
			*/
			...
			add : function(dataObj){
				out("\n# add (rangeCount) : ", this._selection.rangeCount);
				if(this._selection.rangeCount < 1 || this._selection.isCollapsed){
					out("# 선택된 객체 없음");
					return;
				}
			...
			_addAnchorNode: function(anchorID, range){
				...
				var focusNode = this._selection.focusNode;
				var focusOffset = this._selection.focusOffset;
			...
			_divideAnchorRange: function(anchorID, root){
				...
				$anchor = $(root).find(selector).filter(function(){
			...
			**************************************************/
			...
			_isSelectExceptionTag: function(node){
				if($(node).is("select") || $(node).is("option")){
					return true;
				}
			...
			*/
			...
			getMarkTags: function(anchorID){
				...
				var self = this;
				$filterMark = $mark.filter(function(){
			...
			*/
			...
			_xpath2dom : function (xpath){
				...
				// selector 형식 작성
				// /HTML[1]/BODY[1]/DIV[3]/DIV[2]/H4[2]/text()[2] 
			...
			*/
			...
			// 끝나는 경우 normalize가 정상적으로 이루어지지 않고 문자가 삭제되는 현상이 발견되었다.(IE3)
			...
			_normalizeTest: function(){
				...
				// IE에서 "12"로 출력되는 경우가 있다. (IE11 Versions: 11.0.3)
